 ...
 * @brief Responsible for creating and managing grammar items and performing checks on grammars.
 */
struct GrammarFactory
{
    ...
    /**
     * @struct FactoryItem
     * @brief Represents an individual grammar item with its associated symbol table.
     */
    struct FactoryItem
    {
        /**
         * @brief Stores the grammar rules where each key is a non-terminal symbol and each value is a vector of production rules.
         */
        ...
        /**
         * @brief Constructor that initializes a FactoryItem with the provided grammar.
        ...
        /**
         * @brief Checks if a grammar has an empty production for a given non-terminal.
         * @param antecedent The non-terminal symbol to check for empty productions.
    ...
    /**
     * @brief Initializes the GrammarFactory and populates the items vector with initial grammar items.
     */
    void Init();

    /**
     * @brief Picks a random grammar based on the specified difficulty level (1, 2, or 3).
    ...
    /**
     * @brief Generates a Level 3 grammar by combining a Level 2 item and a Level 1 item.
    ...
    /**
     * @brief Checks if a grammar contains unreachable symbols (non-terminals that cannot be derived from the start symbol).
    ...
    /**
     * @brief Checks if a grammar is infinite, meaning there are non-terminal symbols that can never derive a terminal string.
     * This happens when a production leads to an infinite recursion or an endless derivation without reaching terminal symbols.
    ...
    /**
     * @brief Checks if a grammar contains direct left recursion (a non-terminal can produce itself on the left side of a production in one step).
     * @param grammar The grammar to check.
    ...
    /**
     * @brief A vector of FactoryItem objects representing different level 1 grammar items created by the Init method.
     */
